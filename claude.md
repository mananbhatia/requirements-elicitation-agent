...
**Few-shot exemplars in EVAL_PROMPT**: `evaluate_turn()`'s prompt embeds 7 practitioner-annotated
worked examples (3 mistakes — Types 3, 6, 7 — plus 4 well-formed) under a "Worked examples" header,
used to calibrate strictness. Types 1, 2, 4, 5 have no exemplar and are judged from their
definitions alone. Exemplars are sourced from `evals/path_a_fewshot_package.json` (gold-set pool
turns where both practitioners independently agreed). Turns from the held-out test split must never
appear as exemplars — earlier same-session consultant turns that are test turns are replaced with
neutral bracketed summaries in the exemplar prior contexts. Because these live in `EVAL_PROMPT`,
few-shot is always on in production (original turns and Stage C alternatives alike). The zero-shot
vs few-shot comparison is run offline via `evals/fewshot_comparison.py`.
...
- **CHARACTER** (always visible): `Identity`, `Maturity Level`, `Team Members`, `Personality and Communication Style`, `Company Overview`, `Character Knowledge`
  - `Character Knowledge` stays as a blob in `character_text` — NOT parsed into retrievable chunks. `scenario.character_knowledge` is empty for legacy files; embedding retrieval for CK is only active in multi-persona format.
- **SURFACE** (gated): `What the Client Can Articulate` → `discovery_items`
- **TACIT** (gated): `What the Client Knows But Won't Volunteer [Tacit Knowledge]` → `discovery_items`
  - Both surface and tacit map to the same `discovery_items` list. The surface/tacit distinction is not preserved in the Scenario object.
...
- API keys (`ANTHROPIC_API_KEY`, `DATABRICKS_TOKEN`) are stored as Databricks secrets
...
- `python -m pytest tests/test_embedding_retrieval.py -v -k "not smoke"` — run pre-filter + EmbeddingStore unit tests (no API key needed)
- `python run_embedding_test.py` — smoke-test embedding retrieval with DEBUG scoring logs; useful for threshold calibration
- `python replay_session.py logs/session_YYYY-MM-DD_HH-MM-SS.json` — replay a saved session log in the Streamlit UI (evaluation phase, no LLM calls)
- `python evals/stability_test.py` — run the production evaluator N times per gold-set turn (self-consistency; thesis Chapter 7)
- `python evals/evaluation_analysis.py` / `python evals/fewshot_comparison.py` — reproduce the LLM-as-judge agreement and few-shot analyses
...
├── replay_session.py                # replay a saved session log in the Streamlit UI (evaluation phase only, no LLM calls)
...
│   ├── gold_set_manifest.json       # 42-turn gold set: researcher key + turn source (used for stratification)
│   ├── annotation_sheet_*.xlsx      # expert annotator sheets (Paul, Joost, domain_expert)
│   ├── stability_test.py            # runs the production evaluator N times per gold-set turn; measures self-consistency
│   ├── evaluation_analysis.py       # normalisation, 14/28 train/test split, Cohen's kappa, zero-shot accuracy
│   ├── fewshot_comparison.py        # zero-shot vs few-shot evaluator comparison on 28-turn held-out test set
│   └── path_a_fewshot_package.json  # source of the EVAL_PROMPT worked examples (few-shot pool)
├── figures/
...
│   │   └── archive/                 # superseded single-persona scenario files
